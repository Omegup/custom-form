...
| `getFlatInsertionIndex.ts` | **`getFlatInsertionIndex(sectionFlatIndex, columns, colIndex)`** — column slot → flat insertion index (used by `edit-section` add dropdowns) |
...
<EditFormTest
  ...
  renderAddItem={({ sIndex, insertionIndex }) => (
    <AddDropdown onPick={(newItem) =>
      openItemEditor(openFormItemInsertSession(newItem, { index: insertionIndex, sIndex }))
    } />
  )}
...
`renderAddItem` renders a "+" control at the end of every column slot; the host
computes `insertionIndex` via `getFlatInsertionIndex`. `renderLayout` replaces
the default stacked layout (alert / details / sections) so demos can place a
sidebar. The same two props exist on the multi-type list shell
`FormItemEditorFormTest` (form-item-editor demo), which the side-menu and
edit-section stories compose.
